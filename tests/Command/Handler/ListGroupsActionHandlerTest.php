...
use Tourze\LarkAppBotBundle\Command\Handler\ListGroupsActionHandler;
 ...
 * @internal
 */
#[CoversClass(ListGroupsActionHandler::class)]
#[RunTestsInSeparateProcesses]
final class ListGroupsActionHandlerTest extends AbstractIntegrationTestCase
{
    ...
    {
        $handler = self::getService(ListGroupsActionHandler::class);
        $this->assertSame('list', $handler->getActionName());
    }

    public function testExecuteWithNoGroups(): void
    {
        $groupService = $this->createMock(GroupService::class);
        $groupService->method('listGroups')->willReturn([
            'items' => [],
            'has_more' => false,
        ]);
        ...
        $input = new ArrayInput(['--page' => '1', '--size' => '20'], $this->createInputDefinition());
