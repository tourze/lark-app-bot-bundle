...
use Tourze\LarkAppBotBundle\Command\Handler\UpdateGroupActionHandler;
 ...
 * @internal
 */
#[CoversClass(UpdateGroupActionHandler::class)]
#[RunTestsInSeparateProcesses]
final class UpdateGroupActionHandlerTest extends AbstractIntegrationTestCase
{
    ...
    {
        $handler = self::getService(UpdateGroupActionHandler::class);
        $this->assertSame('update', $handler->getActionName());
    }

    public function testExecuteRequiresChatId(): void
    {
        $groupService = $this->createMock(GroupService::class);
        $infoHandler = $this->createMock(ShowGroupInfoActionHandler::class);
        self::getContainer()->set(GroupService::class, $groupService);
        self::getContainer()->set(ShowGroupInfoActionHandler::class, $infoHandler);
        $handler = self::getService(UpdateGroupActionHandler::class);

        $input = new ArrayInput(['--name' => 'New Name'], $this->createInputDefinition());
    ...
    public function testExecuteUpdatesGroup(): void
    {
        $groupService = $this->createMock(GroupService::class);
        $groupService->expects($this->once())->method('updateGroup');
        ...
        $input = new ArrayInput(['--name' => 'Updated Name'], $this->createInputDefinition());
        $output = new BufferedOutput();
        $io = new SymfonyStyle($input, $output);
